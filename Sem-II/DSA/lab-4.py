# Implement stack using doublyl linked list 
class Node:
...
class Stack:
    ...
    def push(self, data):
        ...
        if self.head is None:   # If the stack is empty, set the new node as the head
            ...
            new_node.next = self.head  # If the stack is not empty, set the new node as the new head
            self.head.prev = new_node  # Link the new node to the previous head
            self.head = new_node
    ...
    def pop(self):
        if self.is_empty():
            ...
            if self.head:
                self.head.prev = None  # Remove the reference to the popped node
